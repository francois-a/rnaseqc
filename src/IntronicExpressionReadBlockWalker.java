...
public class IntronicExpressionReadBlockWalker extends CountReadBlockMetricsWalker {
    ...
    public void initialize() {
        ...
        try{
            ...
            /* *    Transcript ID  Number of reads in exons Total exon length Exon Reads per base Number of reads in introns
            ...
            intronOnlyReportFile  = this.intronReportFile+"_intronOnly.txt";
            ...
            exonOnlyReportFile  = this.intronReportFile+"_exonOnly.txt";
...
//            System.out.println("New Contig: " + lastContig);
            shedAll();
        }
...
//
                if(splitRead && splitDosage !=null){
                    if(intronNumber <0 ){
                        ...
                        //System.out.println("Dosage: " + refSeq.getTranscriptId() + " (" +intronNumber + ") : " + dosage);
                        splitDosage.put(intronNumber, dosage);
                    }else{
                        //System.out.println("Killing dosage " + intronNumber);
                        splitDosage = null;
                    }
                }
...
//                }
            ...
            try{
                ...
                if (keep) addRefSeqToMetrics(refSeq, intronNumber, read.getDuplicateReadFlag(),splitRead, splitDosage);
...
//                System.out.println("\t\tBlock was NOT a junction");
        ...
        if (wasIntragenicRead){
            ...
            if (wasExonicRead && !wasJunction){
                ...
                intronOrUTR++; // was an intron or juction
            }
        }
    ...
    public void onTraversalDone(Integer result) {
        ...
        try{
            ...
            while (line != null){
                ...
                float rpkm = Transcript.getRPKM(exonReads, exonLength, this.totalExonQuantifiedReads); // stirct mode support
    ...
    private int overlapsIntron(RefSeqFeature refSeq, GenomeLoc blockLoc) {
        ...
        for (GenomeLoc exon: refSeq.getExons()){
            ...
            if (blockLoc.getStart() < exon.getStart()){
                ...
                return i-1;
            }else if (blockLoc.getStop() <= exon.getStop()){
                // our read is in the exon
    ...
    private void addRefSeqToMetrics(RefSeqFeature refSeq, int intronNumber, boolean wasDuplicate, boolean wasSplit, HashMap<Integer,Float> dosage) {
        ...
        //System.out.println("Added " +refSeq.toString());
    ...
    private int inExon(RefSeqFeature refSeq, GenomeLoc readLoc) {
        ...
        for (GenomeLoc exon: refSeq.getExons()){
            i++;
            if (readLoc.getStart() >= exon.getStart()
                    && readLoc.getStop() <= exon.getStop()){
                return i;
            }
        }
    ...
    class TranscriptMetrics {
        ...
        public void countIntronExon(int intronNumber, boolean wasDuplicate, boolean wasSplit, HashMap<Integer,Float> dosage) {
            if (intronNumber < 0){
                ...
                totalExonReadCount++;
                if (wasSplit && dosage!=null){
                    //System.out.println("Using Dosage");
